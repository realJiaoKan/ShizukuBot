...
samewidthFontpath = "assets/Monaco.ttf"
otherFontpath = "assets/msyh.ttc"
fontpath = otherFontpath


def drawText(img_pil, text, offset_x):
...
def text2pic(text):
    ...
    for j in range(len(text_list)):
        ...
        if len(text) > 0 and text[0] == '|':
            text = text[1:]
            font = ImageFont.truetype(samewidthFontpath, 24)
        else:
            font = ImageFont.truetype(otherFontpath, 24)
        draw.text((padding, padding + j * (margin + h)), text, font=font, fill=(0, 0, 0))
    return i


def pic2base64(img, format='PNG'):
